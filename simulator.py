...
class MinHeap(object):
    ...
    def add(self, process):        
        ...
        # print("Add %d to heap" % process.id)
        # [print(x) for x in self.__list]
...
def SRTF_scheduling(process_list):
    ...
    waiting_time = 0
    sched_proc_inx = getNewlyArrivedProcess(sched_proc_inx, proc_list_to_sch, current_time, minheadp)
        ...
        if next_arrival_time < 0:            
            ...
            if proc_to_exe.burst_time <= next_arrival_time - current_time:                
                ...
                if minheadp.get_min() is None:
                    ...
                    sched_proc_inx = getNewlyArrivedProcess(sched_proc_inx+1, proc_list_to_sch, current_time, minheadp)                    
            # Upon the time next process arrives, need to recheck the SRT.
                ...
                current_time = next_arrival_time                                
                sched_proc_inx = getNewlyArrivedProcess(sched_proc_inx+1, proc_list_to_sch, current_time, minheadp)                                        
    avg_waiting_time = waiting_time/(len(proc_list_to_sch))
    return schedule,avg_waiting_time 
...
def getNewlyArrivedProcess(scheduled_process_index, process_list_to_sch, current_time, minheadp):
    for i in range(scheduled_process_index, len(process_list_to_sch)):
        if process_list_to_sch[i].arrive_time <= current_time:
            minheadp.add(process_list_to_sch[i])
